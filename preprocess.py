...
if __name__ == "__main__":
    ...
    test_list_np = np.reshape(test_list_arr, (total_testing_participant,trial_per_subj))

    ##################### epoching all training and testing data ###################
    train_data_list = np.empty((0, stimulus_per_subj, len(channels), epoch_len), float)
    test_data_list = np.empty((0, stimulus_per_subj, len(channels), epoch_len), float)
    ...
    for testing_participant_id in range(total_testing_participant):
        ...
        for trial_id in range(trial_per_subj):
            subject_dir = subject_dir_list[trial_id]
            data = generate_epoch('FeedBackEvent', './data/test/'+subject_dir, channels, fs, lowcut, highcut, epoch_s, epoch_e, bl_s, bl_e)
            subject_epoch = np.vstack((subject_epoch, data))
        subject_epoch = np.reshape(subject_epoch, (1, stimulus_per_subj, len(channels), epoch_len))
    ...
    X_train = XC.fit_transform(np.reshape(train_data_list, (total_training_participant*stimulus_per_subj, len(channels), epoch_len)), y_train)
    X_train = TangentSpace(metric='riemann').fit_transform(X_train)
    X_test = XC.transform(np.reshape(test_data_list, (10*stimulus_per_subj, len(channels), epoch_len)))
