...
from scipy import butter, sosfiltfilt
...
if __name__ == "__main__":
    ...
    train_list_np = np.reshape(
        train_list_arr, (total_training_participant, trial_per_subj))
    test_list_arr = np.array(sorted(listdir('./data/test')))
    test_list_np = np.reshape(
        test_list_arr, (total_testing_participant, trial_per_subj))
    ...
    for training_participant_id in range(total_training_participant):
        ...
        for trial_id in range(trial_per_subj):
            ...
            data = generate_epoch('FeedBackEvent', './data/train/'+subject_dir, channels,
                                  butter_bandpass_filter, fs, lowcut, highcut, epoch_s, epoch_e, bl_s, bl_e)
