    ...
    for estimator in estimators:
        ...
        if type(preds) is type(None):
            ...
            if predictions.shape != preds.shape:
                ...
                raise Exception(" predictions' shape not equal among estimators within the  batch as %d!=%d " % (
...
class StackNetClassifier(BaseEstimator, ClassifierMixin):
    ...
    def __init__(self, models, metric="logloss", folds=3, restacking=False, use_retraining=True, use_proba=True, random_state=12345, n_jobs=1, verbose=0):
        ...
        if not isinstance(models, list):
            raise Exception(
                "Models has to be a list of sklearn type of models ")
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        if len(classes) <= 1:
            raise Exception(
                "Number of classes must be at least 2, here only %d was given " % (len(classes)))

        self.classes_ = classes
        self.n_classes_ = len(self.classes_)

        if isinstance(self.folds, int):
            indices = KFold(n_splits=self.folds, shuffle=True,
                            random_state=self.random_state).split(y)
        ...
        for level in range(len(self.models)):
            ...
            if not type(previous_input) is type(None) and self.restacking:
                if self._sparse:
                    ...
                    current_input = np.column_stack(
                        (previous_input, current_input))

            if self.verbose > 0:
                print("Input Dimensionality %d at Level %d " %
                      (current_input.shape[1], level))

            this_level_models = self.models[level]

            if self.verbose > 0:
                print("%d models included in Level %d " %
                      (len(this_level_models), level))
            ...
            for train_index, test_index in indices:
                ...
                for d in range(len(this_level_models)):
                    ...
                    if self.use_proba:
                        ...
                        if hasattr(this_model, 'predict_proba'):
                            preds_transformed = predict_from_broba(
                                predictions_[d])
                            metrics_i[d] = self.metric(
                                y_cv, preds_transformed, sample_weight=w_cv)
                            metrics[d] += metrics_i[d]
                            if self.verbose > 0:
                                print("Level %d, fold %d/%d , model %d , %s===%f " %
                                      (level, i+1, iter_count, d, self.metric_name, metrics_i[d]))
                        elif self.n_classes_ == 2 and hasattr(this_model, 'predict'):
                            preds_transformed = predict_from_broba(
                                predictions_[d])
                            metrics_i[d] = self.metric(
                ...
                if self._level_dims[level] != preds_concat_.shape[1]:
                    raise Exception("Output dimensionality among folds is not consistent as %d!=%d " % (
                        self._level_dims[level], preds_concat_.shape[1]))
                train_oof[test_index] = preds_concat_
                if self.verbose > 0:
                    print(
                        "=========== end of fold %i in level %d ===========" % (i+1, level))
                i += 1
            ...
            if self.verbose > 0:
                print("Output dimensionality of level %d is %d " %
                      (level, current_input.shape[1]))

            end_of_level_time = time.time()
            if self.verbose > 0:
                print(
                    "====================== End of Level %d ======================" % (level))
                print(" level %d lasted %f seconds " %
    ...
    def predict_proba(self, X):
        ...
        if X.shape[1] != self.n_features_:
            raise Exception("Input dimensionality of %d is not the same as the trained one with %d " % (
        ...
        for level in range(len(self.estimators_)):
            ...
            if self.verbose > 0:
                print("%d estimators included in Level %d " %
            ...
            if test_pred.shape[1] != self._level_dims[level]:
                raise Exception("Output dimensionality for level %d with %d is not the same as the one during training with %d " % (
